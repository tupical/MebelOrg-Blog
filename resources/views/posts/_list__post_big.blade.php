<a href="{{ route('posts.show', $post)}}">
    <div class="news news__big"  style="background-image: url(@if ($post->image_preview) {{asset('/storage/images/post/' . $post->image_preview)}} @else /storage/images/post/default.jpg @endif);" >
        <!---<img class="news__img" src="img/news-big-1.png" alt="Advise">-->
        <div class="news__label">@if (isset($post->category->id)){{ $post->category->name }}@endif</div>
        <div class="news__wrap_big">
            <div class="news__title-wrap news__title-wrap_big">
                <h3 title="{{ $post->title }}" class="news__title news__title_big">{{ $post->title }}</h3>
            </div>
